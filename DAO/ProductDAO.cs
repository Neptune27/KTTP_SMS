...
namespace DAO
{
    public class ProductDAO
    {
        ...
        public string getBarcode(string productID)
        {
            SqlConnection con = DatabaseHelper.getConnection();
            string barcode = "";
            try
            {
                con.Open();
                SqlCommand cmd = new SqlCommand($"select Barcode from Product where ProductId = '{productID}'", con);
                // Khai báo DataTable để lưu kết quả từ câu truy vấn
                DataTable dt = new DataTable();
                SqlDataAdapter adp = new SqlDataAdapter(cmd);
                adp.Fill(dt);
                // Kiểm tra xem có bất kỳ dòng dữ liệu nào được trả về hay không
                if (dt.Rows.Count > 0)
                {
                    // Nếu có dòng dữ liệu được trả về, lấy giá trị barcode từ cột đầu tiên của dòng đó
                    barcode = dt.Rows[0]["Barcode"].ToString();
                }
            }
            catch (SqlException ex)
            {
                // In lỗi ra console để kiểm tra và gỡ rối
        ...
        public DataTable getProductBySupplierID(string supplierID)
        {
            SqlConnection con = DatabaseHelper.getConnection();
            DataTable dt = new DataTable();
            try
            {
                con.Open();
                SqlCommand cmd = new SqlCommand($"select ProductId, ProductName, ImportPrice, Quantity from Product where SupplierID = '{supplierID}'", con);
                SqlDataAdapter adp = new SqlDataAdapter(cmd);
                adp.Fill(dt);
            }
            catch (SqlException)
        ...
        public bool insertProduct(ProductDTO newProductDTO)
        {
            ...
            try
            {
                ...
                cmd.Parameters.AddWithValue("@Barcode", newProductDTO.Barcode);
                con.Open();
                cmd.ExecuteNonQuery();
                commit = true;
